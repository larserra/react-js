...
const Register = () => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    const { name, email, password, isStaff } = value;
    if (!email || !password || (!isStaff && !name)) {
      toast.warning("please fill out all fields");
    }
  };

  const toggleInfo = () => {
    setValue({ ...value, isStaff: !value.isStaff });
  };
